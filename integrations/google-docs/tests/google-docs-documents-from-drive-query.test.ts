...
import createSync from '../syncs/documents-from-drive-query.js';

describe('google-docs documents-from-drive-query tests', () => {
    const models = 'DriveQueryDocument'.split(',');

    const createTestContext = () => {
        const nangoMock = new global.vitest.NangoSyncMock({
            dirname: __dirname,
            name: 'documents-from-drive-query',
            Model: 'DriveQueryDocument'
        });
