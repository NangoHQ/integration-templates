...
// Operation: contacts/list-topics
const InputSchema = z
    .object({ contact_id: z.string(), limit: z.number().int().min(1).max(100).optional(), after: z.string().optional(), before: z.string().optional() })
    .passthrough()
...
const action = createAction({
    description:
        'Retrieve topics for a contact in Resend. Returns one page; pass next_cursor back as after, or as before when paginating backwards, to continue.',
    version: '1.0.0',
    input: InputSchema,
    output: OutputSchema,
    scopes: [],
