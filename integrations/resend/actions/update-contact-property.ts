...
// Operation: contact-properties/update
const InputSchema = z.object({ id: z.string(), body: z.object({ fallback_value: z.union([z.string(), z.number()]).optional() }).passthrough() }).passthrough();

const ProviderResponseSchema = z.object({ id: z.string().optional(), object: z.string().optional() }).passthrough();
const OutputSchema = ProviderResponseSchema;

const action = createAction({
    description: 'Update an existing contact property in Resend.',
    version: '1.0.0',
    input: InputSchema,
    output: OutputSchema,
    scopes: [],
