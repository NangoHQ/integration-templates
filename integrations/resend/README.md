...
`create-contact-import` takes the CSV contents as text in `body.file` and sends them as the provider's `multipart/form-data` upload; the other form fields (`column_map`, `on_conflict`, `segments`, `topics`) are JSON-encoded form parts. Keep the CSV within the action input size limits.
