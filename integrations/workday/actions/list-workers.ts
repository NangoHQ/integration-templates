...
const action = createAction({
    ...
    exec: async (nango, input): Promise<z.infer<typeof OutputSchema>> => {
        ...
        do {
            ...
            for (const worker of workers) {
                ...
                if (currentPosition?.Manager_Reference) {
                    managerId =
                        findId(currentPosition.Manager_Reference.ID, 'Employee_ID') || findId(currentPosition.Manager_Reference.ID, 'Contingent_Worker_ID');
                }
            ...
            hasMoreData = res.Response_Results.Page < totalPages;
            page += 1;
        } while (hasMoreData);
        ...
        if (hasMoreData) {
            result.next_cursor = page.toString();
        }
