...
const ChannelSchema = z.object({
    ...
    name_normalized: z.union([z.string(), z.null()]),
    created: z.number(),
...
const sync = createSync({
    ...
    version: '1.0.0',
    endpoints: [{ method: 'POST', path: '/syncs/sync-channels', group: 'Channels' }],
    ...
    exec: async (nango) => {
        const metadata = (await nango.getMetadata()) as MetadataType;
        const checkpoint = (await nango.getCheckpoint()) as CheckpointType | undefined;
        ...
        while (currentTypeIndex < typesToFetch.length) {
            ...
            const channels: ChannelType[] = response.data.channels.map((rawChannel: any) => ({
                id: rawChannel.id,
                name: rawChannel.name ?? null,
                name_normalized: rawChannel.name_normalized ?? null,
                created: rawChannel.created,
                creator: rawChannel.creator ?? null,
                ...
                topic: {
                    ...
                    creator: rawChannel.topic?.creator ?? null,
                    last_set: rawChannel.topic?.last_set ?? 0
                },
                purpose: {
                    value: rawChannel.purpose?.value ?? '',
                    creator: rawChannel.purpose?.creator ?? null,
