...
import createSync from '../syncs/shared-sites-selection.js';

describe('sharepoint-online shared-sites-selection tests', () => {
  const models = 'SharedSiteFile'.split(',');

  const createTestContext = () => {
    const nangoMock = new global.vitest.NangoSyncMock({
      dirname: __dirname,
      name: "shared-sites-selection",
      Model: "SharedSiteFile"
    });
