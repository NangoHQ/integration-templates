...
const InputSchema = z.object({
    calendar_id: z
        .string()
        .describe('Calendar identifier. To retrieve calendar IDs call the calendarList.list method. Use "primary" for the primary calendar.'),
    channel_id: z.string().describe('A unique UUID or similar unique string that identifies this channel.'),
    webhook_url: z.string().url().describe('The URL where notifications will be delivered.'),
...
const OutputSchema = z.object({
    ...
    resource_uri: z.string().describe('A version-specific canonical URL for the watched resource.'),
    token: z.union([z.string(), z.null()]).describe('The token sent in the request (if any).'),
    expiration: z.union([z.string(), z.null()]).describe('Expiration time as a Unix timestamp (long), or null if no expiration.')
});
...
const action = createAction({
    ...
    exec: async (nango, input): Promise<z.infer<typeof OutputSchema>> => {
        ...
        const response = await nango.post({
            endpoint: `calendar/v3/calendars/${encodeURIComponent(input.calendar_id)}/events/watch`,
            data: {
                id: input.channel_id,
                type: 'web_hook',
                address: input.webhook_url,
