 ...
 * @returns {TeamsList[]} - The list of team IDs and names.
 */
export function findTeamFields(data: any): TeamsList[] {
    ...
    Object.entries(editMetaFields).forEach(([fieldId, fieldMeta]: [string, any]) => {
        ...
        if (meta.schema?.configuration?.['com.atlassian.jira.plugin.system.customfieldtypes:atlassian-team']) {
            const fieldValue = data.fields?.[fieldId];
            if (fieldValue && 'id' in fieldValue && 'name' in fieldValue) {
                teams.push({
                    teamId: fieldValue.id,
                    teamName: fieldValue.name
                });
            }
        }
    });
