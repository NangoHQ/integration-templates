...
const InputSchema = z.object({
    id: z.string().describe('The unique identifier of the subaccount to delete. Example: "cust-123"')
});
...
const action = createAction({
    description: 'Delete an existing subaccount.',
    version: '1.0.0',
    input: InputSchema,
    output: OutputSchema,
    ...
    exec: async (nango, input): Promise<z.infer<typeof OutputSchema>> => {
        const response = await nango.post({
            // https://mailchimp.com/developer/transactional/api/subaccounts/delete-subaccount/
            endpoint: '/1.0/subaccounts/delete',
            data: {
                id: input.id
            },
            retries: 10
