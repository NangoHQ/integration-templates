...
const InputSchema = z.object({
    ...
    phone_number: z.string().optional().describe('Required only when the code was sent to a number you provided. E.164 format (e.g. +14155550100).')
});
...
const action = createAction({
    ...
    exec: async (nango, input): Promise<z.infer<typeof OutputSchema>> => {
        const response = await nango.post({
            // https://docs.agentcard.sh/api-reference/wallet/verify-phone-code
