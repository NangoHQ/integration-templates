...
export async function* paginate(
  ...
  while (true) {
    ...
    if (
      !response?.data ||
      !response.data[responseDataPath as keyof NamelyAPIProfileResponse]
    ) {
    ...
    if (lastSyncDate) {
      ...
      if (filteredProfiles.length === 0) {
        await nango.log(
          "Reached profiles older than lastSyncDate, stopping pagination.",
        );
        break;
      }
    }
    ...
    if (profiles.length < limit) {
      await nango.log(
        "Received fewer results than limit, pagination complete.",
      );
      break;
    }
