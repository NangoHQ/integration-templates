...
const sync = createSync({
  description:
    "Fetches a list of current employees from Namely and maps them to the standard HRIS model",
  version: "0.0.1",
  frequency: "every 1h",
  autoStart: true,
  ...
  exec: async (nango) => {
    ...
    for await (const { profiles, groups } of paginate(
      nango,
      paginationParams,
    )) {
      const standardEmployees = profiles.map((profile) =>
        toStandardEmployee(profile, groups),
      );
