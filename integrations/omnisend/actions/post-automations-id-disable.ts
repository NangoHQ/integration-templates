...
const input = z.object({ id: z.string().min(1), body: z.object({ contactsInWorkflow: z.enum(['keep', 'exit']) }).passthrough() }).passthrough();
const output = z
    .object({
        blocks: z
            .array(
...
const action = createAction({
    ...
    input,
    output,
    exec: async (nango, requestInput) => output.parse((await callOmnisend(nango, 'POST', '/automations/{id}/disable', requestInput)).data)
});
