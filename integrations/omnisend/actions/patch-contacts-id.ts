...
const input = z.object({"id": z.string().min(1), body: z.unknown()}).passthrough();
...
const action = createAction({
    ...
    (await callOmnisend(nango, 'PATCH', '/contacts/{id}', requestInput as Record<string, unknown>)).data
    )
});
