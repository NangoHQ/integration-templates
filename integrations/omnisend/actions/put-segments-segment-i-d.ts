...
const input = z.object({"segmentID": z.string().min(1), body: z.object({"conditionGroups": z.array(z.object({"conditions": z.unknown().optional()}).passthrough()).optional(), "name": z.string().optional()}).passthrough()}).passthrough();
...
const action = createAction({
    ...
    (await callOmnisend(nango, 'PUT', '/segments/{segmentID}', requestInput as Record<string, unknown>)).data
    )
});
