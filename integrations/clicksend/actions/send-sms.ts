import type { ClickSendSmsInput, NangoAction, Sms } from '../../models';
import { toSms } from '../mappers/to-sms.js';
import type { ClickSendSms } from '../types.js';

export default async function runAction(nango: NangoAction, input: ClickSendSmsInput): Promise<Sms> {
    const payload = {
        messages: [
            {
                to: input.to,
                body: input.body
            }
        ]
    };

    const response = await nango.proxy({
        // https://developers.clicksend.com/docs/messaging/sms/other/send-sms
        endpoint: '/sms/send',
        method: 'POST',
        data: payload,
        retries: 3
    });

    const clickSendSms: ClickSendSms = response.data?.data?.messages?.[0];

    return toSms(clickSendSms);
}
