...
const InputSchema = z.object({
    ...
    output_expires_after: z
        .object({
            anchor: z.literal('created_at').describe('The file creation timestamp anchors the expiration policy.'),
            seconds: z.number().int().min(3600).max(2592000).describe('Seconds until output and error files expire, from 1 hour to 30 days.')
        })
        .optional()
        .describe('Optional expiration policy for output and error files generated by the batch.')
});
    ...
    exec: async (nango, input): Promise<z.infer<typeof OutputSchema>> => {
        const response = await nango.post({
            ...
            data: {
                ...
                ...(input.output_expires_after !== undefined && { output_expires_after: input.output_expires_after })
            },
            retries: 3
        });
