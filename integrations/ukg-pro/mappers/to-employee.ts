...
export function toEmployee(response: EmployeeDetails): StandardEmployee {
    ...
    const manager: Person | undefined =
        sanitizeString(response.supervisorId) && sanitizeString(response.supervisorName)
            ? {
                  id: sanitizeString(response.supervisorId),
                  firstName: sanitizeString(response.supervisorName.split(' ')[0]),
                  lastName: sanitizeString(response.supervisorName.split(' ').slice(1).join(' ')),
                  email: ''
              }
            : undefined;
    ...
    const employee: StandardEmployee = {
        ...
        displayName: [sanitizeString(response.firstName), middleName, sanitizeString(response.lastName)].filter(Boolean).join(' '),
        ...
        employmentType: mapEmploymentType(sanitizeString(response.employeeType), sanitizeString(response.fullTimeOrPartTime)),
        employmentStatus: mapEmploymentStatus(sanitizeString(response.employeeStatus), response.isActive),
        ...
        workLocation: {
            name: sanitizeString(response.workLocation) || sanitizeString(response.companyName),
            type: 'OFFICE',
 ...
 */
function mapEmploymentType(employeeType: string, fullTimeOrPartTime: string): StandardEmployee['employmentType'] {
 ...
 */
function mapEmploymentStatus(employeeStatus: string, isActive: boolean): StandardEmployee['employmentStatus'] {
