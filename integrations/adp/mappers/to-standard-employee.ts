...
function mapEmploymentType(workerTypeCode: { codeValue?: string; longName?: string; shortName?: string } | undefined): StandardEmployee['employmentType'] {
    if (!workerTypeCode?.codeValue) return 'OTHER';

    const type = [workerTypeCode?.longName?.toLowerCase(), workerTypeCode?.shortName?.toLowerCase(), workerTypeCode?.codeValue?.toLowerCase()]
        .filter(Boolean)
        .join(' ')
        .replace(/-|_/g, ' ')
        .replace(/\s+/g, ' ')
    ...
    if (type.includes('freelancer') || type.includes('contractor') || type.includes('consultant')) return 'CONTRACTOR';
...
function mapEmploymentStatus(statusCode: { codeValue?: string; shortName?: string } | undefined): StandardEmployee['employmentStatus'] {
    if (!statusCode?.codeValue) return 'PENDING';

    const status = statusCode.shortName?.toLowerCase() || statusCode.codeValue.toLowerCase();
...
export function toStandardEmployee(employee: ADPEmployee): StandardEmployee {
    ...
    const jobTitle = workAssignment?.jobTitle || workAssignment?.jobCode?.shortName || '';
    ...
    const departmentName =
        workAssignment?.occupationalClassifications?.[0]?.classificationCode?.shortName ||
        workAssignment?.occupationalClassifications?.[0]?.nameCode?.shortName ||
        'Unknown Department';

    // Extract termination information
    const terminationDate = parseDate(workAssignment?.terminationDate || employee.workerDates?.terminationDate);
    const terminationReason = workAssignment?.assignmentStatus?.reasonCode?.shortName || '';
    ...
    const baseEmployee: StandardEmployee = {
        ...
        displayName: legalName?.formattedName || `${legalName?.givenName || ''} ${legalName?.familyName1 || ''}`.trim(),
        ...
        employmentStatus: mapEmploymentStatus(workAssignment?.assignmentStatus?.statusCode),
        ...
        addresses: [
            ...(legalAddress
                ? [
                      {
                          street: `${legalAddress.lineOne || ''} ${legalAddress.lineTwo || ''}`.trim(),
        ...
        providerSpecific: {
            ...
            preferredGenderPronounCode: employee.person?.preferredGenderPronounCode?.shortName,
        ...
        updatedAt: parseDate(workAssignment?.terminationDate || employee.workerDates?.terminationDate) || ''
    };
