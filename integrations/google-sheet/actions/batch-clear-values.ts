...
const InputSchema = z.object({
    spreadsheet_id: z.string().describe('The ID of the spreadsheet to update. Example: "1a2b3c4d5e6f"'),
...
const action = createAction({
    ...
    exec: async (nango, input): Promise<z.infer<typeof OutputSchema>> => {
        ...
        const response = await nango.post({
            endpoint: `/v4/spreadsheets/${input.spreadsheet_id}/values:batchClear`,
            data: {
                ranges: input.ranges
            },
            retries: 3
