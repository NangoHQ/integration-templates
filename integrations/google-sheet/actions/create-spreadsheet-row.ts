...
const InputSchema = z.object({
    ...
    sheet_id: z.number().describe('The ID of the sheet (can be found via the API). Example: 687284948'),
    sheet_name: z.string().describe('The name of the sheet for A1 notation. Example: "Sheet1"'),
    row_index: z.number().describe('The index where to insert the row (0-based, where 0 is the first row). Example: 5'),
...
const action = createAction({
    ...
    exec: async (nango, input): Promise<z.infer<typeof OutputSchema>> => {
                ...
                requests: [
                    {
                        insertDimension: {
                            range: {
                                ...
                                dimension: 'ROWS',
                                startIndex: input.row_index,
                                endIndex: input.row_index + 1
                            },
        ...
        const range = `${input.sheet_name}!A${input.row_index + 1}:${endColumn}${input.row_index + 1}`;
        ...
        return {
            ...
            values: input.values,
            updated_range: updateResponse.data?.updatedRange || range
        };
    }
});
