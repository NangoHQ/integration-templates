...
import { getJson, kpiChannelResponseWithName, kpiQuery } from '../shared.js';
...
const action = createAction({
    ...
    exec: async (nango, values) =>
        kpiChannelResponseWithName.parse(await getJson(nango, `/analytics/api/v1/kpis/channels/${encodeURIComponent(values.channel)}/`, values))
});
