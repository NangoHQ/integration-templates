...
import createSync from '../syncs/free-text-invoices.js';

describe('microsoft-dynamics-365-finance-and-operations-cc free-text-invoices tests', () => {
  const models = 'FreeTextInvoice'.split(',');

  const createTestContext = () => {
    const nangoMock = new global.vitest.NangoSyncMock({
      dirname: __dirname,
      name: "free-text-invoices",
      Model: "FreeTextInvoice"
    });
