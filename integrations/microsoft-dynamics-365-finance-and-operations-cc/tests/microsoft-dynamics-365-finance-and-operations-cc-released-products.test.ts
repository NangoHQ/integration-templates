...
import createSync from '../syncs/released-products.js';

describe('microsoft-dynamics-365-finance-and-operations-cc released-products tests', () => {
  const models = 'ReleasedProduct'.split(',');

  const createTestContext = () => {
    const nangoMock = new global.vitest.NangoSyncMock({
      dirname: __dirname,
      name: "released-products",
      Model: "ReleasedProduct"
    });
