...
import { toStandardEmployee, toStandardEmployeeDetailed } from '../mappers/to-standard-employee.js';
...
const sync = createSync({
    ...
    exec: async (nango) => {
        ...
        for await (const paycomEmployeesResponseArray of nango.paginate<PaycomEmployee>(directoryConfig)) {
            await nango.log(`Processing ${paycomEmployeesResponseArray.length} employees from directory`);
            ...
            for (const employee of paycomEmployeesResponseArray) {
                ...
                try {
                    ...
                    if (detailedResponse.data) {
                        const standardEmployee = toStandardEmployeeDetailed(detailedResponse.data.data[0]);
                        ...
                        currentBatch.push(standardEmployee);
                        await nango.log(`Fallback to basic data for employee ${employee.eecode}`, { level: 'warn' });
                    }
                } catch (error: any) {
                    // If detailed fetch fails, use basic employee data
                    const standardEmployee = toStandardEmployee(employee);
                    currentBatch.push(standardEmployee);
                    await nango.log(`Error fetching detailed data for employee ${employee.eecode}: ${error}`, { level: 'warn' });
                }
                ...
                if (currentBatch.length >= batchSize) {
                    ...
                    await nango.log(`Saved batch of ${currentBatch.length} employees. Total processed: ${totalEmployeesProcessed}`);
        ...
        if (currentBatch.length > 0) {
            await nango.batchSave(currentBatch, 'StandardEmployee');
            totalEmployeesProcessed += currentBatch.length;
            await nango.log(`Saved final batch of ${currentBatch.length} employees. Total processed: ${totalEmployeesProcessed}`);
        }
